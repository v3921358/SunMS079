...
public abstract class MapleEvent {
    ...
    public void givePrize(final MapleCharacter chr) {
        ...
        if (reward == 0) {
            chr.gainMeso(88888, true);
            chr.dropMessage(5, "你获得 88888 冒险币");
        } else if (reward == 1) {
            chr.gainMeso(199999, true);
            chr.dropMessage(5, "你获得 199999 冒险币");
        } else if (reward == 2) {
        ...
        if (MapleInventoryManipulator.checkSpace(chr.getClient(), 4031250, 500, "")) {
            MapleInventoryManipulator.addById(chr.getClient(), 4031250, (short) 500);
            chr.dropMessage(5, "你获得 500 个盛大易宝");
        } else {
            // givePrize(chr); //do again until they get
            chr.gainMeso(100000, true);
        ...
        if (auto) {
            for (MapleEventType t : MapleEventType.values()) {
                ...
                if (e.isRunning) {
                    for (int i : e.mapid) {
                        if (cserv.getEvent() == i) {
                            e.broadcast(CWvsContext.serverNotice(0, "距离活动开始只剩下一分钟!"));
    ...
    public static final String scheduleEvent(final MapleEventType event, final ChannelServer cserv) {
        ...
        World.Broadcast.broadcastMessage(CWvsContext.serverNotice(0, "活动 " + String.valueOf(event) + " 在频道 " + cserv.getChannel() + " 准备开始 , 要参加的玩家请到频道 " + cserv.getChannel() + ".请找到活动向导珀儿进入！"));
        World.Broadcast.broadcastMessage(CWvsContext.serverNotice(0, "活动 " + String.valueOf(event) + " 在频道 " + cserv.getChannel() + " 准备开始 , 要参加的玩家请到频道 " + cserv.getChannel() + ".请找到活动向导珀儿进入！"));
        World.Broadcast.broadcastMessage(CWvsContext.serverNotice(0, "活动 " + String.valueOf(event) + " 在频道 " + cserv.getChannel() + " 准备开始 , 要参加的玩家请到频道 " + cserv.getChannel() + ".请找到活动向导珀儿进入！"));
        return "";
    }
