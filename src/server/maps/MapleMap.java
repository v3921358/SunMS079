...
public final class MapleMap {
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob, final boolean instanced) {
        ...
        //int mesos = Math.min(30000, (int) (mesoDecrease * (mob.getMobExp()) * (1.0 + Math.random() * 20) / 20.0));
        int tempmeso = Math.min(30000, (int) (mesoDecrease * (mob.getStats().getExp()) * (9.0 + Math.random() * 5) / 10.0));
        if (tempmeso > 0) {
            pos.x = Math.min(Math.max(mobpos - 25 * (d / 2), footholds.getMinDropX() + 25), footholds.getMaxDropX() - d * 25);
            spawnMobMesoDrop((int) (tempmeso * (chr.getStat().mesoBuff / 100.0) /**
                     * chr.getDropMod()
                     */
                    * cmServerrate), calcDropPos(pos, mob.getTruePosition()), mob, chr, false, droptype);
            //mesoDropped = true;
        }
                ...
                for (final MapleCharacter mc : characters) {
                    if (mc.isAlive()) {
                        ...
                        switch (monster.getId()) {
                            ...
                            case 8810122:
                            case 8820001:
                                mc.getClient().getSession().write(EffectPacket.showOwnBuffEffect(buffid, 11, mc.getLevel(), 1)); // HT nine spirit
                                broadcastMessage(mc, EffectPacket.showBuffeffect(mc.getId(), buffid, 11, mc.getLevel(), 1), false); // HT nine spirit
                                break;
                        }
                    }
                }
...
//            FileoutputUtil.log(FileoutputUtil.Zakum_Log, MapDebug_Log());
            ...
            doShrine(true);
            /*
             * } else if (mobid >= 9400903 && mobid <= 9400910) { boolean
             * makeZakReal = true; final Collection<MapleMonster> monsters =
             * getAllMonstersThreadsafe();
             *
             * for (final MapleMonster mons : monsters) { if (mons.getId() >=
             * 9400903 && mons.getId() <= 9400910) { makeZakReal = false; break;
             * } } if (makeZakReal) { for (final MapleMapObject object :
             * monsters) { final MapleMonster mons = ((MapleMonster) object); if
             * (mons.getId() == 9400900) { final Point pos =
             * mons.getTruePosition(); this.killAllMonsters(true);
             * spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9400900),
             * pos); break; } } }
             */
    ...
    public final void updateMonsterController(final MapleMonster monster) {
        ...
        try {
            ...
            while (ltr.hasNext()) {
                chr = ltr.next();
                if (!chr.isHidden() && !chr.isClone() && (chr.getControlledMonsters().size() < mincontrolled || mincontrolled == -1) && chr.getTruePosition().distanceSq(monster.getTruePosition()) <= monster.getRange()) {
     ...
     * monster.getStats().getSummonType() <= 1 ||
     * monster.getStats().getSummonType() == 27 || overwrite ? spawnType :
    ...
    public final void spawnMonster(final MapleMonster monster, final int spawnType, final boolean overwrite) {
        ...
        if (monster.getId() == 9300166) {
            ...
            }, this.getId() == 109010100 ? 1500 : this.getId() == 910025200 ? 200 : 3000);
        }
        spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
...
//	broadcastMessage(CField.dropItemFromMapObject(mdrop, mob.getTruePosition(), dropPos, (byte) 0));
    ...
    public final void addPlayer(final MapleCharacter chr) {
        ...
        for (final MaplePet pet : chr.getPets()) {
            if (pet.getSummoned()) {
                ...
                chr.getClient().getSession().write(PetPacket.loadExceptionList(chr, pet));
            }
        }
        if (hasBoat() == 2) {
            chr.getClient().getSession().write(CField.boatPacket(true));
        } else if (hasBoat() == 1 && (chr.getMapId() != 200090000 || chr.getMapId() != 200090010)) {
            chr.getClient().getSession().write(CField.boatPacket(false));
        }
        if (!chr.isClone()) {
        ...
        /*
         * if (!chr.isInBlockedMap() && chr.getLevel() > 10 && isTown() &&
         * chr.getMapId() % 1000000 == 0 && chr.getMapId() <= 600000000) { for
         * (QuickMove qm : QuickMove.values()) { if (qm.getMap() ==
         * chr.getMapId()) { List<QuickMoveNPC> qmn = new LinkedList(); int npcs
         * = qm.getNPCFlag(); for (QuickMoveNPC npc : QuickMoveNPC.values()) {
         * if ((npcs & npc.getValue()) != 0 && npc.show()) { qmn.add(npc); } }
         * chr.getClient().getSession().write(CField.getQuickMoveInfo(true,
         * qmn)); break; } } } else {
         * chr.getClient().getSession().write(CField.getQuickMoveInfo(false, new
         * LinkedList())); }
         */
        ...
        if (!chr.isClone()) {
            if (chr.getChalkboard() != null) {
                chr.getClient().getSession().write(CSPacket.useChalkboard(chr.getId(), chr.getChalkboard()));
            }
    ...
    public void getRankAndAdd(String leader, String time, ExpeditionType type, long timz, Collection<String> squad) {
        try {
            ...
            try (Connection con = DBConPool.getInstance().getDataSource().getConnection(); PreparedStatement ps = con.prepareStatement("INSERT INTO speedruns(`type`, `leader`, `timestring`, `time`, `members`) VALUES (?,?,?,?,?)")) {
            ...
            System.err.println("getRankAndAdd" + e);
    ...
    public void spawnBomb(final MapleCharacter chr) {
        final MapleMonster bomb = MapleLifeFactory.getMonster(9300166);
        bomb.changeLevel(250, true);
        spawnMonster_sSack(bomb, chr.getPosition(), -2);
        Timer.EventTimer.getInstance().schedule(new Runnable() {

            @Override
            public void run() {
                killMonster(bomb, chr, false, false, (byte) 1);
            }
        }, 10 * 1000);
    }
    ...
    public void AutoNx(int dy) {
        ...
        for (MapleCharacter chr : characters) {
            if (ServerConfig.pdexp > 0) {
                expdy = ServerConfig.pdexp * chr.getLevel() * 15;
