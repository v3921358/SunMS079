...
public class MapleInventoryManipulator {
    ...
    public static void addRing(MapleCharacter chr, int itemId, int ringId, int sn, String partner) {
        ...
        // chr.getClient().getSession().write(CSPacket.sendBoughtRings(InventoryConstants.isCrushRing(itemId), ring, sn, chr.getClient().getAccID(), partner));
    }
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (!GameConstants.isRechargable(item.getItemId())) {
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (oldQ < slotMax && item.getOwner().equals(eItem.getOwner()) && item.getExpiration() == eItem.getExpiration()) {
                                ...
                                eItem.setQuantity(newQ);
                                if (isPetPickup) {
                                    c.getSession().write(InventoryPacket.addInventorySlot(type, eItem, false));
                                } else {
                ...
                while (quantity > 0) {
                    ...
                    quantity -= newQ;
                    final Item nItem = new Item(item.getItemId(), (short) 0, newQ, item.getFlag());
                    ...
                    if (isPetPickup) {
                        c.getSession().write(InventoryPacket.addInventorySlot(type, nItem, false));
                    } else {
                ...
                // Throwing Stars and Bullets - Add all into one slot regardless of quantity.
                final Item nItem = new Item(item.getItemId(), (short) 0, quantity, item.getFlag());
                nItem.setExpiration(item.getExpiration());
                nItem.setOwner(item.getOwner());
                nItem.setPet(item.getPet());
                nItem.setGMLog(item.getGMLog());
            ...
            if (quantity == 1) {
                ...
                if (isPetPickup) {
                    c.getSession().write(InventoryPacket.addInventorySlot(type, item, false));
                } else {
                    c.getSession().write(InventoryPacket.addInventorySlot(type, item, true));
                }
                if (GameConstants.isHarvesting(item.getItemId())) {
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (c.getPlayer().getDebugMessage()) {
            c.getPlayer().dropMessage("穿上装备: src : " + src + " dst : " + dst + " ID：" + source.getItemId());
        }
                ...
                if (s.id.contains(source.getItemId())) {
                    ...
                    for (Integer i : s.id) {
                        if (theList.contains(i)) {
                            c.getPlayer().dropMessage(1, "你没有装备这个物品，因为你已经有一个 " + (StringUtil.makeEnumHumanReadable(s.name())) + "装备.");
                            c.getSession().write(CWvsContext.enableActions());
                            return;
                        }
                    }
        ...
        for (Item copyStorageItem : c.getPlayer().getStorage().getItems()) {
            if (copyStorageItem != null) {
                if (c.getPlayer().getStorage().removeItemByInventoryItemId(inventoryitemid)) {
                    String msgtext = "玩家" + c.getPlayer().getName() + " ID: " + c.getPlayer().getId() + " (等级" + c.getPlayer().getLevel() + ") 地图: " + c.getPlayer().getMapId() + " 在玩家穿戴中发现复制装备[" + ii.getName(copyEquipedItems.getItemId()) + "]已经将其删除。";
                    World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice("[GM密语] " + msgtext));
