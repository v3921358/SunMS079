...
public class MapleAESOFB {
    ...
    // 一共256位，提供秘钥。
    private final static SecretKeySpec originalSkey = new SecretKeySpec (new byte[] {
            0x13, 0x00, 0x00, 0x00,
            0x08, 0x00, 0x00, 0x00,
            0x06, 0x00, 0x00, 0x00,
    ...
    public MapleAESOFB(byte iv[], short mapleVersion) {
        try {
            ...
            cipher.init(Cipher.ENCRYPT_MODE, originalSkey);
        } catch (NoSuchAlgorithmException | NoSuchPaddingException e) {
//            System.err.println("ERROR" + e);
            e.printStackTrace ();
        } catch (InvalidKeyException e) {
            System.err.println("Error initalizing the encryption cipher. " +
                    "Make sure you're using the Java Cryptography Extension (JCE) " +
    ...
    public static final void funnyShit(byte inputByte, byte[] in) {
        byte a = in[1];
        byte b = inputByte;
        //byte c =originalFunnyBytes[(int) a & 0xFF];
        byte c = funnyBytes[(int) a & 0xFF];
        c -= inputByte;
        in[0] += c;
        c = in[2];
        //c ^= originalFunnyBytes[(int) b & 0xFF];
        ...
        c = a;
        a -= (int) in[0] & 0xFF;
        //c = originalFunnyBytes[(int) c & 0xFF];
        c = funnyBytes[(int) c & 0xFF];
        c += inputByte;
        c ^= in[2];
        in[2] = c;
        //a += (int) originalFunnyBytes[(int) b & 0xFF] & 0xFF;
