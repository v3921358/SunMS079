...
function action(mode, type, selection) {
    ...
    if (status == 0) {
	cm.sendNext("哇，欢迎来到嵩山整形手术馆 我是#p9310036# 如果你有 #b#t5152044##k, 我可以帮你整形。");
    } else if (status == 1) {
	...
	cm.askAvatar("来吧~选择你想要的脸型",facetype);

    } else if (status == 2) {
	if (cm.setAvatar(5152044, facetype[selection]) == 1) {
	    cm.sendOk("享受!");
	} else {
	    cm.sendOk("你好像没有#t5152044#");
	}
	cm.dispose();
    }
}