...
function action(mode, type, selection) {
	...
	if( cm.getPlayer().getBossLog("haogan"+cm.getChar().getMapId())>0){
		haogan=0;
	}
    if (status == 0 && mode == 0) {
        cm.dispose();
    ...
    if (status == 0) {
		if( cm.getPlayer().getBossLog("haogandt")==0){
		...
		if( cm.getPlayer().getBossLog("haogandt")==0){
			if(selection==ass){
				status=-1;
				cm.getPlayer().setBossLog("haogandt");
			...
			if( cm.getPlayer().getBossLog("haogan"+cm.getChar().getMapId())==0){
				cm.getPlayer().setBossLog("haogan"+cm.getChar().getMapId());
