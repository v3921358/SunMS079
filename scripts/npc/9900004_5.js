...
function action(mode, type, selection) {
    ...
    if (status == 0) {
	cm.sendSimple("          #b一线海们#k,在这里可以兑换相应的#r副本勋章#k哟 . \r\n  #L1#兑换月秒副本勋章#v1142014#需要#v4001101#5个 and #v2140000# 5万#k#l\r\n  #L2#兑换废弃副本勋章#v1142026#需要#v4002000#20个 and #v2140000#15万#k#l\r\n  #L3#兑换玩具副本勋章#v1142065#需要#v4002001#20个 and #v2140000#80万#k#l\r\n  #L4#兑换女神副本勋章#v1142067#需要#v4002002#20个 and #v2140001#200万#k#l\r\n  #L5#兑换海盗副本勋章#v1142079#需要#v4002003#20个 and #v2140002#500万#k#l\r\n\r\n\r\n   ");
    } else if (status == 1) {
	if (selection == 0) {
		...
		cm.getC().getSession().write(CWvsContext.InventoryPacket.addInventorySlot(type, toDrop, false)); //刷新背包				
				...
				cm.sendOk("金钱不够或者#v4001101#不够5个!");
				cm.dispose();
			}		
	    }else if(selection == 2){
	    if (cm.haveItem(4002000, 20)&&cm.getMeso() >= 150000) { 
		...
		cm.getC().getSession().write(CWvsContext.InventoryPacket.addInventorySlot(type, toDrop, false)); //刷新背包				
				...
				cm.sendOk("金钱不够或者#v4002000#不够20个!");
				cm.dispose();
			}		
	    }else if(selection == 3){
	    if (cm.haveItem(4002001, 20)&&cm.getMeso() >= 800000) {
		...
		cm.getC().getSession().write(CWvsContext.InventoryPacket.addInventorySlot(type, toDrop, false)); //刷新背包				
				...
				cm.sendOk("金钱不够或者#v4002001#不够20个!");
				cm.dispose();
			}
	    }else if(selection == 4){
	    if (cm.haveItem(4002002, 20)&&cm.getMeso() >= 2000000) { 
		...
		cm.getC().getSession().write(CWvsContext.InventoryPacket.addInventorySlot(type, toDrop, false)); //刷新背包				
				...
				cm.sendOk("金钱不够或者#v4002003#不够20个!");
				cm.dispose();
			}		
		}
    } else if (status == 30){
