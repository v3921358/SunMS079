...
function action(mode, type, selection) {
	...
	if (mode == 1)
		...
		status--;
	if (cm.getChar().getMapId() != 500000000 && cm.getChar().getMapId() != 551000000 && cm.getChar().getMapId() != 702000000 && cm.getChar().getMapId() != 800000000 && cm.getChar().getMapId() != 600000000 && cm.getChar().getMapId() != 540000000 && cm.getChar().getMapId() != 550000000 && cm.getChar().getMapId() != 541000000) {
		if (status == 0) {
			status = 1;
			if (cm.getJob() == 0) {
				cm.sendNext("为了从繁忙的日常中解脱，去享受一趟旅游怎么样？不仅可以体验新颖的异国文化，还能学到不少东西的机会！我们冒险岛旅游公司为您准备了，丰富有趣的#b世界旅游#k套餐,有点贵哦,有钱就可以带你环游世界.");
			} else {
				cm.sendNext("为了从繁忙的日常中解脱，去享受一趟旅游怎么样？不仅可以体验新颖的异国文化，还能学到不少东西的机会！我们冒险岛旅游公司为您准备了，丰富有趣的#b世界旅游#k套餐,有点贵哦,有钱就可以带你环游世界");
			}
		} else if (status == 1) {
...
var selStr = "现在就可以去往 #b泰国的水上市场,少林寺,日本古代神社#k游览一番。在各旅游地我都会为大家提供满意热诚的服务。那么请准备好，新手可以9折优惠。#b";
		...
		if (status == 0) {
			cm.sendSimple ("世界旅游怎么样？很有趣吧。\r\n#L0##b返回：#m" + cm.getSavedLocation("MULUNG_TC") + "# #k#l\r\n#L1##b继续观光#k#l");
		} else if (status == 1) {
			if (selection == 0) {
