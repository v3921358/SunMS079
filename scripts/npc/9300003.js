...
function action(mode, type, selection){	
	...
	if (cm.getPartyFormID() == -4) {
			cm.sendOk("获取伴侣出错!");
            cm.dispose();
			return;
						} else if (cm.getPartyFormID() == -1) { // 不是队长
                cm.sendOk("你们还没有组队！");
                cm.dispose();
				return;
            ...
            //cm.getChannelServer().getPlayerStorage().getCharacterById(cm.getPartyFormID())
            //var gender = cm.getPlayer().getGender();
            var mapId = cm.getPlayer().getMapId();
			 var victim = cm.getChannelServer().getPlayerStorage().getCharacterById(cm.getPartyFormID());
/*             var next = true;
           ...
           if(cm.getChannelServer().getPlayerStorage().getCharacterById(cm.getPartyFormID()).getMapId() != mapId){
                cm.sendNext("您的伴侣不在本地图");
                cm.dispose();
                return;
			}
			...
			cm.getPlayer().setMarriageId(cm.getPartyFormID());
			victim.setMarriageId(cm.getPlayer().getId());
			//cm.givePartyItems(1112804, 1, false);
            cm.makeRing(1112804, victim.getName());
			cm.getPlayer().saveToDB(false, false);
			Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.packet.CWvsContext.broadcastMsg(11, cm.getClient().getChannel(), "-婚姻祝福-" + " : " + "(" + cm.getChar().getName() + ")和他的伴侣("+victim.getName()+")结此终身之盟,守此终身之誓,不离不弃、白头偕老，我们祝他/她们吧！"));
