...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (cm.haveItem(4031256, 1) &&cm.getMeso() >=1) {
                cm.gainItem(4031256,-1);
                cm.gainMeso(-1);
		var ii = MapleItemInformationProvider.getInstance();		                
		var type = ii.getInventoryType(1142210); //获得装备的类形
		var toDrop = ii.randomizeStats(ii.getEquipById(1142210)).copy(); // 生成一个Equip类
		//toDrop. setFlag(1);//上锁
		toDrop. setStr(5);//给力量
		toDrop. setDex(5);//给敏捷 
		toDrop. setInt(5);//给智力
