...
function action(mode, type, selection) {
    if (mode == -1) {
	    ...
	    cm.sendOk("您属于飞侠职业,但是您已经成功三转了,已经超越了教官的强大了!");
	    cm.dispose();
	    return;
		}
            if (!(cm.getJob()==410 ||cm.getJob()==420)) {
				cm.sendOk("请找您的转职教官,您不属于飞侠职业!");
                cm.dispose();
                return;
			} else if (cm.getPlayer().getLevel() < 70) {
				cm.sendOk("你的等级尚未满达到70级");
				cm.dispose();
				return;		
            }	
			if (cm.isQuestActive(100111)){
                ...
                cm.sendOk("看起来你并没有资格转职，去找#r达克鲁#k他会帮助你的!");
                cm.dispose();
            ...
            if (cm.haveItem(4031058, 1) && cm.isQuestActive(100113)) {
                if (cm.getJob()==410) {
					...
					cm.sendOk("恭喜转职了!");
					cm.worldMessage("恭喜玩家 - "+cm.getChar().getName()+"成功三转无影人让我们热烈的祝福他/她吧！");
                    cm.dispose();
					...
					cm.gainItem(4031058, -1);
                    cm.sendOk("恭喜转职了!");
					cm.worldMessage("恭喜玩家 - "+cm.getChar().getName()+"成功三转独行客让我们热烈的祝福他/她吧！");
                    cm.dispose();
                }
           ...
           if (!cm.isQuestFinished(100111)) {
			   cm.forceStartQuest(100112); //开始任务
                cm.sendOk("去找神圣的石头测验吧!!位于#b雪原圣地#k");
                cm.dispose();
            }
        }
    }
