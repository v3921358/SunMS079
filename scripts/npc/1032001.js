...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getJob() == 0) {
            ...
            if (cm.getPlayer().getLevel() >= 30 && cm.getJob() == 200) { // 法师
				...
				cm.sendOk("你完成了一个考验，现在去找 #鲁碧#k.位于冰封雪域#b长老公馆#k!");
				cm.dispose();
				} else if (!cm.isQuestActive(100110)) {
				cm.sendOk("请先去去找 #r鲁碧#k.接受相关任务在来找我!");
				cm.dispose();
			    ...
			    cm.sendOk("你完成了一个考验，现在去找 #b鲁碧#k.位于冰封雪域#b长老公馆#k!");
                   ...
                   //cm.warp(211000001, 0);
                    ...
                    cm.sendOk("嗨,#b#h0##k!我需要一个#b黑符#k.快去#r巫婆森林2#k找#r异次元空间#k拿给我.");
                }
                cm.dispose();
            } else {
	...
	} else if (cm.getJob() == 0 && cm.getPlayer().getLevel() >= 8) {
            cm.changeJob(200); // 法师
			cm.getPlayer().gainSP((cm.getPlayer().getLevel()-8)*3);//转职给技能点
			cm.resetStats(4, 4, 20, 4);
        }
