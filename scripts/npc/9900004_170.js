...
function action(mode, type, selection) {
    ...
    if (status == 0) {
	cm.sendSimple("   在这里可以领取到本周排行第一的勋章,人气,金钱,等级\r\n           #b 每周一更新一次,勋章有效只有7天 #k\r\n#L1#人气排行榜第一  -  #r超级人气王勋章#k#v1142075##l \r\n#L2#金钱财富榜第一  -  #r财富拥有者勋章#k#v1142296##l\r\n#L3#等级荣耀榜第一  -  #r爆肝达人勋章#k#v1142288##l\r\n");
    } else if (status == 1) {
	if (selection == 0) {
		...
		cm.gainItem(3994026, -1);//扣除猪物品		
		var ii = MapleItemInformationProvider.getInstance();		                
		var type = ii.getInventoryType(1142075); //获得装备的类形
		var toDrop = ii.randomizeStats(ii.getEquipById(1142075)).copy(); // 生成一个Equip类
		toDrop. setStr(5);//给力量
        toDrop. setDex(5);//给敏捷 
		toDrop. setInt(5);//给智力
		toDrop. setLuk(5);//给运气
		...
		cm.getC().getSession().write(CWvsContext.InventoryPacket.addInventorySlot(type, toDrop, false)); //刷新背包				
				cm.sendOk("恭喜兑换成功");
				cm.worldMessage(6,"玩家：["+cm.getName()+"] 成功的兑换超级人气王勋章,人气排行榜第一");
				cm.dispose();
			}else{
				cm.sendOk("你不是排行第一的玩家!");
				cm.dispose();
			}		
	    }else if(selection == 2){
	    if (cm.haveItem(3994027, 1)) { 
		cm.gainItem(3994027, -1);//扣除猪物品
	    cm.gainDJ(1000);//卷		
		var ii = MapleItemInformationProvider.getInstance();		                
		var type = ii.getInventoryType(1142296); //获得装备的类形
		var toDrop = ii.randomizeStats(ii.getEquipById(1142296)).copy(); // 生成一个Equip类
		toDrop. setStr(1);//给力量
        toDrop. setDex(1);//给敏捷 
		toDrop. setInt(1);//给智力
		toDrop. setLuk(1);//给运气
		toDrop. setWdef(50);//防御
		toDrop. setHands(10);//手技		
		cm.getPlayer().getInventory(type).addItem(toDrop);//将这个装备放入包中
		cm.getC().getSession().write(CWvsContext.InventoryPacket.addInventorySlot(type, toDrop, false)); //刷新背包				
				cm.sendOk("恭喜兑换成功");
				cm.worldMessage(6,"玩家：["+cm.getName()+"] 成功的兑换财富拥有者勋章,财富排行榜第一!");
				cm.dispose();
			}else{
				cm.sendOk("你不是排行第一的玩家!!");
				cm.dispose();
	    ...
	    if (cm.haveItem(3994028, 1)) { 
		cm.gainItem(3994028, -1);//扣除猪物品	
		var ii = MapleItemInformationProvider.getInstance();		                
		var type = ii.getInventoryType(1142288); //获得装备的类形
		var toDrop = ii.randomizeStats(ii.getEquipById(1142288)).copy(); // 生成一个Equip类
	    toDrop. setHp(0);//HP
        toDrop. setMp(0);//MP
		toDrop. setWatk(6);//攻击力
		toDrop. setMatk(6);//魔法力
		toDrop. setHands(10);//手技			
		cm.getPlayer().getInventory(type).addItem(toDrop);//将这个装备放入包中
		cm.getC().getSession().write(CWvsContext.InventoryPacket.addInventorySlot(type, toDrop, false)); //刷新背包				
				cm.sendOk("恭喜兑换成功");
				cm.worldMessage(6,"玩家：["+cm.getName()+"] 成功的兑换爆肝达人勋章,等级排行榜第一!");
				cm.dispose();
			}else{
				cm.sendOk("你不是排行第一的玩家!!");
				cm.dispose();
