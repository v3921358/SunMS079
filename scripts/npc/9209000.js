...
function action(mode, type, selection) {
    ...
    if(status == 0){
        ...
        if (selection == 0){
            ...
            var text = "";
            for(var i = 0; i < 5; i++)
                text += "目前的市场价格为 #t" + (i + 3994090) + "# 是 #r180#k 的金币\r\n";
            cm.sendNext(text);
            cm.dispose();
        }else
            cm.sendNext("我购买的产品在冒险岛周末市场和其他城镇卖给他们。我换纪念品，香料，动物标本鲨鱼，还有更多...但没有蛋.");
    }else if(status == 1){
        if(sel == 0){
            if(cm.haveItem(3994090 + selection)){
                pickup = 3994090 + selection;
                cm.sendYesNo("目前的价格为180金币。你想现在把它卖掉？"); //Make a price changer by hour.
            }else{
                cm.sendNext("你没有任何东西。别再浪费我的时间......我是一个忙碌的人。");
                cm.dispose();
            }
        }else
            cm.sendNextPrev("冒险岛星期日市场是我的休息日。如果你需要看我，你将不得不前来周一至周五...");
    }else if(status == 2){
        if(sel == 0)
            cm.sendGetNumber("你想要卖多少个?", 0, 0, 200);
        else{
            cm.sendPrev("哦，价格也可能发生变化。我不能让棒的短端，我要留在企业！检查回来我频繁，我的价格变化按每小时变化!");
        }
    }else if(status == 3){
        if(sel == 0)
            if(selection != 1)
